...
public:
    void Set_Size(int x, int y)
    {
        ...
        Grid_Size[1] = y;
        Building temp;
        for (int x = 0; x < Grid_Size[0] * Grid_Size[1]; x++)
        {
            Building_list.push_back(temp);
            Highlight_Data.push_back(0);
        }
        Size[0]=Grid_Size[0]*7-1;
        Size[1]=Grid_Size[1]*7-1;
    }
...
void Map_Grid::Reset(){
    Building Null;
    Null.type(6);
    for (int id = 0; id < Grid_Size[0] * Grid_Size[1]; id++)
    {
        vector<int> temp{id};
        Set_Building(temp, Null);
    }
}
...
void Map_Grid::Expand(int x, int y)
{
    ...
    // rightward
    Building temp;
    for (int id_y = Grid_Size[1] - 1; id_y >= 0; id_y--)
    {
        for (int id_x = 0; id_x < x; id_x++)
        {
            Building_list.insert(Building_list.begin() + id_y * Grid_Size[0] + Grid_Size[0], temp);
            Highlight_Data.insert(Highlight_Data.begin() + id_y * Grid_Size[0] + Grid_Size[0], 0);
        }
    }
    Grid_Size[0] += x;
    // downward
    ...
    {
        for (int id_x = 0; id_x < Grid_Size[0]; id_x++)
        {
            Building_list.push_back(temp);
            Highlight_Data.push_back(0);
        }
...
void Map_Grid::Output()
{
    ...
    for (int y = 0; y < Grid_Size[1]; y++)
    {
        ...
        for (int x = 0; x < Grid_Size[0]; x++)
        {
            ...
            Highlight_Data[x + y * Grid_Size[0]] = R_Main.Add_Textbox(1, Highlight_Data[x + y * Grid_Size[0]], Layer_name, Building_list[x + y * Grid_Size[0]].graphic_S, temp_St, temp_En, 0);
        }
    }
    if (TestMod)
    {
...
void Map_Grid::Set_Building(vector<int> xy, Building item)
{
    if (xy.size() == 2)
    {
        ...
        if ((x >= Grid_Size[0]) || (y >= Grid_Size[1]))
        {
            ...
            cout << "input x: " << x << " y: " << y << '\n';
            cout << "Grid size x: " << Grid_Size[0] << " y: " << Grid_Size[1] << '\n';
            exit(0);
        }
        Building_list[Grid_Size[0] * y + x] = item;
    }
    else
    {
